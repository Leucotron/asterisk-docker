...
```
docker run -it --rm leucotron/asterisk:18.11.3
```
...
Para gerar uma nova imagem com nova versão de Asterisk, nesse caso do exemplo abaixo, usamos a versão 18.11.3 alterando a ENV ASTERISK_VERSION

```
docker build -t leucotron/asterisk:18.11.3 .
```
...
```
docker push leucotron/asterisk:18.11.3
```
