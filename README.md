...
```
docker run -it --rm leucotron/asterisk:18.13.0
```
...
Para gerar uma nova imagem com nova versão de Asterisk, nesse caso do exemplo abaixo, usamos a versão 18.13.0 alterando a ENV ASTERISK_VERSION

```
docker build -t leucotron/asterisk:18.13.0 .
```
...
```
docker push leucotron/asterisk:18.13.0
```
