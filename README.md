...
```
docker run -it --rm leucotron/asterisk:22.4.1
```
...
Para gerar uma nova imagem com nova versão de Asterisk, nesse caso do exemplo abaixo, usamos a versão 22.4.1 alterando a ENV ASTERISK_VERSION

```
docker build --progress=plain -t leucotron/asterisk:22.4.1 .
```
...
```
docker push leucotron/asterisk:22.4.1
```
