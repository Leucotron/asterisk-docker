...
```
docker run -it --rm leucotron/asterisk:18.4.0
```
...
Para gerar uma nova imagem com nova versão de Asterisk, nesse caso do exemplo abaixo, usamos a versão 18.4.0 alterando a ENV ASTERISK_VERSION

```
docker build -t leucotron/asterisk:18.4.0 .
```
...
```
docker push leucotron/asterisk:18.4.0
```
