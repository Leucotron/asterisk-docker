...
```
docker run -it --rm leucotron/asterisk:18.7.1
```
...
Para gerar uma nova imagem com nova versão de Asterisk, nesse caso do exemplo abaixo, usamos a versão 18.7.1 alterando a ENV ASTERISK_VERSION

```
docker build -t leucotron/asterisk:18.7.1 .
```
...
```
docker push leucotron/asterisk:18.7.1
```
