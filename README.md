...
```
docker run -it --rm leucotron/asterisk:22.1.0
```
...
Para gerar uma nova imagem com nova versão de Asterisk, nesse caso do exemplo abaixo, usamos a versão 22.1.0 alterando a ENV ASTERISK_VERSION

```
docker build --progress=plain -t leucotron/asterisk:22.1.0 .
```
...
```
docker push leucotron/asterisk:22.1.0
```
