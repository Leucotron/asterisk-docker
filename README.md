...
```
docker run -it --rm leucotron/asterisk:22.2.0
```
...
Para gerar uma nova imagem com nova versão de Asterisk, nesse caso do exemplo abaixo, usamos a versão 22.2.0 alterando a ENV ASTERISK_VERSION

```
docker build --progress=plain -t leucotron/asterisk:22.2.0 .
```
...
```
docker push leucotron/asterisk:22.2.0
```
...
4) Atualizações do Asterisk
Para conferir as atualizações feitas no Asterisk, consulte o site:
https://downloads.asterisk.org/pub/telephony/asterisk/ChangeLog-22-current.md
